...
class TransporterOrdersBloc extends Bloc<TransporterOrdersEvent, TransporterOrdersState> {
  ...
  Future<void> _onFetchOrders(FetchTransporterOrdersEvent event, Emitter<TransporterOrdersState> emit) async {
    ...
    try {
      ...
      emit(TransporterOrdersLoaded(allOrders: allOrders, displayOrders: displayOrders));
    } catch (e) {
      emit(TransporterOrdersError(e.toString()));
    }
  }

  Future<void> _onLoadMoreOrders(LoadMoreOrdersEvent event, Emitter<TransporterOrdersState> emit) async {
    if (state is TransporterOrdersLoaded) {
      final currentState = state as TransporterOrdersLoaded;
      if (currentState.isLoadingMore || currentState.displayOrders.length >= currentState.allOrders.length) {
        return;
      }
      
      emit(currentState.copyWith(isLoadingMore: true));
      
      await Future.delayed(const Duration(milliseconds: 300)); // Simulate UI loading effect
      
      _currentPage++;
      final nextItems = currentState.allOrders.skip(_currentPage * pageSize).take(pageSize).toList();
      final updatedDisplayOrders = List.of(currentState.displayOrders)..addAll(nextItems);
  ...
  Future<void> _onConfirmPickedUp(ConfirmPickedUpEvent event, Emitter<TransporterOrdersState> emit) async {
    try {
  ...
  Future<void> _onConfirmDelivered(ConfirmDeliveredEvent event, Emitter<TransporterOrdersState> emit) async {
    try {
