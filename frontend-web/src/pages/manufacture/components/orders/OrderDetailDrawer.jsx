...
const lineColumns = [
  ...
  { title: 'ID lô (Raw batch)', dataIndex: 'targetRawBatchId', key: 'targetRawBatchId', ellipsis: true },
  {
    title: 'Số lượng đặt',
    key: 'qty',
...
const OrderDetailDrawer = ({ open, loading, order, onClose }) => {
  if (loading && !order) {
    return (
  ...
  return (
    <Drawer title={`Đơn ${order.orderCode || ''}`} placement="right" width={640} open={open} onClose={onClose}>
      <Descriptions bordered column={1} size="small" style={{ marginBottom: 16 }}>
        ...
        <Descriptions.Item label="Người bán (NCC)">
          <UserDirectoryDisplay userId={order.sellerId} />
        </Descriptions.Item>
        <Descriptions.Item label="Ghi chú">{order.note || '—'}</Descriptions.Item>
        <Descriptions.Item label="VC (nếu có)">
      ...
      {isM2S && order.lines?.length > 0 ? (
        ...
        <Empty description="Không có dòng lô" />
      )}
    </Drawer>
  );
};
